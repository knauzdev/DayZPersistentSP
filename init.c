...
void init() {
  lifespanp = PluginLifespan.Cast( GetPlugin( PluginLifespan ) );
  spawnPlayer();
  setWeatherLimits();
  startHive();
  inited = true;
...
void handleInput() {
  if(KeyState(KeyCode.KC_P) && KeyState(KeyCode.KC_LSHIFT)) {
    if(FileExist(playersave) && player) {
      DeleteFile(playersave);
...
void spawnPlayerFromFile() {
  ...
  //FileHandle file = OpenFile("$CurrentDir:\\player.log", FileMode.WRITE);
  ...
  player = PlayerBase.Cast( GetGame().CreatePlayer( NULL, modelType, "0 0 0", 0, "NONE") );
  ...
  player.StatUpdate("playtime", splaytime.ToFloat());
  player.SetLastShavedSeconds(slastshave.ToFloat());
  ...
  lifespanp = PluginLifespan.Cast( GetPlugin( PluginLifespan ) );
  lifespanp.UpdateLifespan( player, true );
  player.SetSynchDirty();

  //Add items
  ItemBase handItem;
  if(inHands != "") {
    handItem = player.GetInventory().CreateInInventory(inHands);
    player.PredictiveTakeEntityToHands(handItem);
    if(handItem == NULL) {
  ...
  for(int i=0; i<inventoryItems.Count(); i++) {
    if(handItem != NULL && handItem.GetInventory().CreateAttachment(inventoryItems[i]) == NULL) { //try to attack to hand weapon
    if(player.GetInventory().CreateInInventory(inventoryItems[i]) == NULL) { //if space in inventory for item, spawn in inventory
        //last resort: spawn on ground
        g_Game.CreateObject( inventoryItems[i], g_Game.GetPlayer().GetPosition(), false );
      }
    }
  }

  player.SetHealth("","", shealth.ToFloat());
  player.SetHealth("GlobalHealth", "Blood", sblood.ToFloat());
  player.GetStatTemperature().Set(stemp.ToFloat());
  player.GetStatEnergy().Set(senergy.ToFloat());
  player.GetStatWater().Set(swater.ToFloat());
}
...
void savePlayerToFile() {
	...
	FPrintln(file, player.GetPosition());
	FPrintln(file, player.GetDirection());
	...
	string modelname = player.GetType();
	...
	float beard = player.GetLastShavedSeconds();
